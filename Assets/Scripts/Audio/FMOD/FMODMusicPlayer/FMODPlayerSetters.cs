...
public partial class FMODMusicPlayer
{
    ...
    {
        RESULT result = _eventInstance.setParameterByName(parameter.ToString(), parameterValue);
        if (result != RESULT.OK)
        {
            throw new Exception($"Error returned when SETTING parameter of FMOD EventInstance: {result}");
        }
    }
